...
import {
  Paths, Uses, Contributions, TextureAll, Path, Use,
} from '~/v2/interfaces';
import {
  mapContributions, mapPaths, mapTexture, mapUses,
...
export default firestorm.collection('textures', (el) => {
  el.uses = async (): Promise<Uses> => uses
    .search([
      {
  ...
  el.paths = async (): Promise<Paths> => el.uses()
  ...
  el.contributions = async (): Promise<Contributions> => contributions
    .search([
      {
