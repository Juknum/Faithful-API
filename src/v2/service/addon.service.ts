...
import {
  Addons, Addon, AddonStatus, AddonAll, AddonRepository, Files, File, FileParent,
} from '../interfaces';
...
export default class AddonService {
  ...
  async getScreenshots(id): Promise<Array<string>> {
    return this.getFiles(id)
      .then((files: Files) => files.filter((f: File) => f.use === 'screenshot' || f.use === 'carousel'), // todo: only keep screenshots
      )
