...
import {
  textures, paths, uses, contributions,
} from '../../firestorm';
import {
  Contributions, Paths, Texture, Textures, TextureAll, Uses, TextureRepository, Path,
} from '../../interfaces';
...
export default class TextureFirestormRepository implements TextureRepository {
  ...
  getEditions = function () {
    return uses
      ...
      .then((res) => Object.values(res).reduce((acc: Array<string>, cur: OldUse) => {
    ...
    return contributions
      ...
      .then((res: any) => (
					Object.values(res).reduce((acc: Array<string>, cur: any) => {
					  const { res } = cur; // TODO: change with resolution
  ...
  getVersions(): Promise<Array<string>> {
    return paths
      ...
      .then((res) => Object.values(res).reduce((acc: Array<string>, cur: Path) => {
